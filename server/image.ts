...
export async function generateImage(
  ...
  if (image) {
    ...
    const christmasPrompt = `${prompt}. Transform this image into a festive Christmas scene while maintaining 
ABSOLUTE FACIAL IDENTITY PRESERVATION. The face, facial features, skin tone, 
eyes, nose, mouth, and facial expression of every person must remain EXACTLY 
as in the original image - completely unchanged and locked. If the original 
background setting is suitable for Christmas decoration, enhance it with 
holiday elements (lights, ornaments, garlands, Christmas tree, snowflakes). 
If the original background is not viable or appropriate for a Christmas scene, 
place the people into a new festive Christmas environment (cozy living room 
with Christmas tree, winter wonderland, Christmas market, decorated home 
interior). Add festive winter clothing elements such as Christmas sweaters, 
Santa hats, scarves, or beanies to the people, but their faces, head shapes, 
and personal identity must stay 100% consistent with the original photograph. 
CRITICAL: Zero facial modifications allowed - only background and clothing 
changes permitted.
`;
